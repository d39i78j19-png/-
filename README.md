...
**`standalone/public/index.html` をブラウザで開くだけ**で使えます。
Node.js もターミナルも APIキーも不要です。

1. 左メニューの **「顔写真プール」** を開く
2. 顔写真シート（セグメントごとのパネルが並んだ1枚の画像）を読み込む
   → シートの構造を**自動検出**し、1枚ずつの切り出し範囲が決まります
3. 枠が顔に合っていることを確認して **「切り出して登録」**
4. ペルソナを生成して **「全員の顔写真を生成」**

ペルソナの **志向（成長／安定）× 志望企業規模（大手／ベンチャー／中小）** と
性別・年齢に合わせて、登録した写真から自動で割り当てられます。
画像処理はすべてブラウザ内で完結し、外部には送信されません。

**他の人に配る**: 「この顔写真を埋め込んだHTMLを書き出す」を押すと、写真が埋め込まれた
HTML が1枚できます。渡せば相手は開くだけで使えます。

AI によるペルソナ本文生成や、画像生成APIでの顔写真作成を使いたい場合のみ、
サーバーを起動します。

```bash
npm run portraits:serve       # http://localhost:8787
```
