...
インストール無しで触ってもらいたい場合は、単一 HTML に固めたデモを書き出せます。

```bash
npm run web:demo   # dist-web/index.html を生成（外部リソース参照なし）
```

本番と同じ `src/app/page.tsx` をそのまま使い、API 呼び出しだけ
`src/demo/apiShim.ts` のルールベース実装に差し替えます（UI 側は分岐なし）。
API キーを預からない前提なので、デモは常にルールベースで動きます。
...
| エンドポイント | 役割 | 生成方式 |
| --- | --- | --- |
| `POST /api/extract` | 掲載文 → company_info | Claude ＋ ルールベース補完（キー無しならルールベースのみ） |
...
## 掲載文の読み取り（Step 1 の貼り付け欄）

採用サイトやマイナビのページを全選択してコピーし、そのまま貼り付けて
**「掲載文を読み取ってフォームに反映」**を押すと、下の各項目が埋まります。
読み取り結果はすべて手で直せます。**上書きされるので、先に貼り付けてから手直ししてください。**

読み取りは「見出しで本文を区切ってから項目を拾う」方式です。

- `■事業内容` `求める人材：` `【勤務地】` のような見出しを辞書で認識し、次の見出しまでを1区画として扱う
- `・` `1.` で始まる行と、句点で終わらない短い行だけを箇条書き項目として拾う
  （「自ら課題を見つけて動ける方を求めています。」のような説明文はスキル欄に入れない）
- 卒業年度は `2027年3月卒業見込` `27卒` の両表記を西暦4桁に揃える
- 社名は `会社名` 欄が無ければ `株式会社◯◯` / `◯◯株式会社` を本文から拾い、文中で拾った場合の助詞（「〜は」）を落とす
- 「リモート」「在宅」「テレワーク」に触れていれば勤務地に `リモート可` を足す

API キーがある場合は Claude が文脈で読み取り、**Claude が埋められなかった項目だけ**を
上のルールベース抽出器が補います（社名と卒業年度は正規表現のほうが安定するため）。
キーが無い場合はルールベースのみで動きます。

読み取れなかった項目は `missing_fields` として返り、画面に「読み取れなかった項目」として
表示されます。見出しが一切ない自由記述の掲載文では、社名・事業内容・勤務地・卒業年度は
拾えますが、バリューや必須スキルは拾えないことが多いので手入力が前提です。

貼り付けた掲載文は Claude への入力ではデータとして扱うようタグで囲んでおり、
掲載文中に指示めいた文が混ざっていても指示として実行しません。
1回の読み取りは先頭 30,000 文字までで、超えた分は切り捨てて警告を出します。
